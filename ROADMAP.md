...
Make Cursor a first-class Kit client: anything a team does in Kit's web app (hire, triage security reports, run outreach, train, review) can be done from the editor through one install, while the plugin itself stays a thin manifest.
...
- **Least privilege.** OAuth scopes per module, writes off by default, risky write tools confirm before they act.
